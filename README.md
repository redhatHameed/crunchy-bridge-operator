...
- For build and push the operator image, operator-bundle image and operator-catalog image to a registry run:  `make release`**make sure change the registry under -  `Makefile` and replace `ORG` in the `IMAGE_TAG_BASE` with your own Quay.io Org!**
